...
function convertToRomanNumerals(num) {
  ...
  for (let i = 0; i < romanMapping.length; i += 1) {
    while (remaining >= romanMapping[i].value) {
      result += romanMapping[i].numeral;
      remaining -= romanMapping[i].value;
    }
  }
...
function sortByAsc(arr) {
  ...
  const sortedLeft = sortByAsc(leftArray);
  const sortedRight = sortByAsc(rightArray);
...
function getNearestBigger(number) {
  ...
  let rest = newNumber % 10;

  array.push(rest);
  newNumber = Math.floor(number / 10);

  while (newNumber % 10 >= rest) {
    rest = newNumber % 10;
    array.push(rest);
  ...
  const part = array[i];
  array[i] = swap;
  newNumber = newNumber * 10 + part;

  for (let j = 0; j < array.length; j += 1) {
    newNumber = newNumber * 10 + array[j];
  }
